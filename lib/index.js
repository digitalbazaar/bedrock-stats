...
async function generateReport() {
  const now = Date.now();
  // pass the timestamp into the handlers
  const monitors = {_createdDate: now};
  await bedrock.events.emit('bedrock-stats.report', monitors);
  delete monitors._createdDate;
  const report = {createdDate: now, monitors};
